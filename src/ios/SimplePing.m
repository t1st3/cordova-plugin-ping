...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        if ( [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber] ) {
            if ( (strongDelegate != nil) && [strongDelegate respondsToSelector:@selector(simplePing:didReceivePingResponsePacket:sequenceNumber:rtt:)] ) {
                receiveDate = [NSDate date];
                
                NSTimeInterval rtt = [receiveDate timeIntervalSinceDate:sendDate];
                packet = [NSMutableData dataWithBytes:buffer length:(NSUInteger) bytesRead  ];
                [strongDelegate simplePing:self didReceivePingResponsePacket:packet sequenceNumber:sequenceNumber rtt:rtt];
            }
        } else {
